...
<p style="text-align: center;">
	...
	<?php echo substr($order->getDateTime(), 0, 10); ?>
</p>
<p></p>
	...
	<tbody>
		<tr>
			<td width="50%">
				...
				<?php echo nl2br(StoreInvoice::fontize(htmlspecialchars(trim(tr_(isset($sellerCompanyInfo) ? $sellerCompanyInfo : ""))))); ?><br />
			</td>
			<td width="50%">
				<strong><?php echo StoreInvoice::fontize(StoreModule::__('Buyer')); ?></strong><br />
				<?php if( $order->getBillingInfo()->isCompany ) { ?>
					<?php echo StoreInvoice::fontize($order->getBillingInfo()->companyName); ?><br />
					<?php echo StoreInvoice::fontize(StoreModule::__('Company Code')); ?>: <?php echo StoreInvoice::fontize($order->getBillingInfo()->companyCode); ?><br />
					<?php echo StoreInvoice::fontize(StoreModule::__('Company TAX/VAT number')); ?>: <?php echo StoreInvoice::fontize($order->getBillingInfo()->companyVatCode); ?><br />
				<?php } else { ?>
					<?php echo StoreInvoice::fontize($order->getBillingInfo()->firstName); ?> <?php echo StoreInvoice::fontize($order->getBillingInfo()->lastName); ?><br />
				<?php } ?>
				<?php echo StoreInvoice::fontize(StoreModule::__('Country')); ?>: <?php echo StoreInvoice::fontize($order->getBillingInfo()->country); ?><br />
				<?php if( $order->getBillingInfo()->region ) { ?>
					<?php echo StoreInvoice::fontize(StoreModule::__('Region')); ?>: <?php echo StoreInvoice::fontize($order->getBillingInfo()->region); ?><br />
				<?php } ?>
				<?php if( $order->getBillingInfo()->city ) { ?>
					<?php echo StoreInvoice::fontize(StoreModule::__('City')); ?>: <?php echo StoreInvoice::fontize($order->getBillingInfo()->city); ?><br />
				<?php } ?>
				<?php if( $order->getBillingInfo()->address2 ) { ?>
					<?php echo StoreInvoice::fontize(StoreModule::__('Address')); ?> 1: <?php echo StoreInvoice::fontize($order->getBillingInfo()->address1); ?><br />
					<?php echo StoreInvoice::fontize(StoreModule::__('Address')); ?> 2: <?php echo StoreInvoice::fontize($order->getBillingInfo()->address2); ?><br />
				<?php } else { ?>
					<?php echo StoreInvoice::fontize(StoreModule::__('Address')); ?>: <?php echo StoreInvoice::fontize($order->getBillingInfo()->address1); ?><br />
				<?php } ?>
				<?php if( $order->getBillingInfo()->postCode ) { ?>
					<?php echo StoreInvoice::fontize(StoreModule::__('Post Code')); ?>: <?php echo StoreInvoice::fontize($order->getBillingInfo()->postCode); ?><br />
				<?php } ?>
				<?php if( $order->getBillingInfo()->email ) { ?>
					<?php echo StoreInvoice::fontize(StoreModule::__('E-mail')); ?>: <?php echo StoreInvoice::fontize($order->getBillingInfo()->email); ?><br />
				<?php } ?>
				<?php if( $order->getBillingInfo()->phone ) { ?>
					<?php echo StoreInvoice::fontize(StoreModule::__('Phone Number')); ?>: <?php echo StoreInvoice::fontize($order->getBillingInfo()->phone); ?><br />
				<?php } ?>
			</td>
		</tr>
	</tbody>
...
<br /><br />
<table border="0" cellpadding="3" style="page-break-inside: avoid;">
	<tbody>
		<?php if( $order->getTaxAmount() || $order->getShippingAmount() ) { ?>
		<tr>
			<td style="width: 85%; text-align: right;"><?php echo StoreInvoice::fontize(StoreModule::__('Subtotal')); ?></td>
			<td style="width: 15%;"><?php echo StoreInvoice::fontize(htmlspecialchars(StoreData::formatPrice($order->getPrice() - $order->getTaxAmount() - $order->getShippingAmount(), $order->getPriceOptions(), $order->getCurrency()))); ?></td>
		</tr>
		<?php } ?>
		<?php if( $order->getTaxAmount() ) { ?>
			<tr>
				<td style="width: 85%; text-align: right;"><?php echo StoreInvoice::fontize(StoreModule::__('Tax')); ?></td>
				<td style="width: 15%;"><?php echo StoreInvoice::fontize(htmlspecialchars(StoreData::formatPrice($order->getTaxAmount(), $order->getPriceOptions(), $order->getCurrency()))); ?></td>
			</tr>
		<?php } ?>
		<?php if( $order->getShippingAmount() ) { ?>
			<tr>
				<td style="width: 85%; text-align: right;"><?php echo StoreInvoice::fontize(StoreModule::__('Shipping')); ?></td>
		...
		<?php } ?>
		<tr>
			<td style="width: 85%; text-align: right;"><?php echo StoreInvoice::fontize(StoreModule::__('Total')); ?></td>
...
<p>
<?php echo nl2br(StoreInvoice::fontize(<<<EOT
