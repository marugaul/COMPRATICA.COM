...
/* =========================
   Sesi¨®n endurecida (unifica nombre con config.php)
   ========================= */
...
/* =========================
   CSRF: token en sesi¨®n + cookie vg_csrf sincronizada
   ========================= */
...
function security_set_csrf_cookie(): void {
  ...
  if ($token === '') return;
  $isHttps = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off');
  // Exponer cookie legible por JS (no httponly) para que el front la reenv¨ªe por header
  setcookie('vg_csrf', $token, [
    'expires'  => time() + 60*60*24, // 1 d¨ªa
    'path'     => '/',
...
function current_user_id(): ?int {
  // Soporta ambas claves de sesi¨®n
