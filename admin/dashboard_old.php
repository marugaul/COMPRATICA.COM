...
if ($action === 'create' || $action === 'update') {
    ...
    if ($action === 'create') {
        ...
        $stmt->execute([$name,$desc,$price,$stock,$imageName,$currency,$active,now_iso(),now_iso()]);
        $msg = "Producto creado.";
    } else {
        if ($imageName) {
            $stmt = $pdo->prepare("UPDATE products SET name=?, description=?, price=?, stock=?, image=?, currency=?, active=?, updated_at=? WHERE id=?");
            $stmt->execute([$name,$desc,$price,$stock,$imageName,$currency,$active,now_iso(),$id]);
        } else {
            $stmt = $pdo->prepare("UPDATE products SET name=?, description=?, price=?, stock=?, currency=?, active=?, updated_at=? WHERE id=?");
            $stmt->execute([$name,$desc,$price,$stock,$currency,$active,now_iso(),$id]);
        }
...
<title>Backoffice - <?php echo APP_NAME; ?></title><link rel="stylesheet" href="../assets/style.css"></head>
<body>
<header class="header"><div class="logo">⚙️ Backoffice</div><nav><a class="btn" href="../index.php">Ver tienda</a><a class="btn" href="logout.php">Salir</a></nav></header>
<div class="container">
...
<h2>Productos</h2>
<table class="table">
  <thead><tr><th>ID</th><th>Imagen</th><th>Nombre</th><th>Precio</th><th>Moneda</th><th>Stock</th><th>Activo</th><th>Acciones</th></tr></thead>
  <tbody>
    <?php foreach($products as $p): $sym = ($p['currency']==='USD')?'$':'₡'; ?>
    <tr>
...
function fillForm(p){
  ...
  document.getElementById('currency').value = p.currency || 'CRC';
