<?php
// admin/empleos_config.php
// Configuración de planes de EMPLEOS
    ...
    if ($action === 'delete_plan') {
        ...
        if ($id > 0) {
            try {
                ...
                $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM job_listings WHERE listing_type='job' AND pricing_plan_id = ?");
                $stmt->execute([$id]);
...
$plans = $pdo->query("
    SELECT
        p.*,
        (SELECT COUNT(*) FROM job_listings WHERE listing_type='job' AND pricing_plan_id = p.id) as listings_count
    FROM job_pricing p
...
<title>Configuración de Planes - Empleos</title>
...
<div class="header">
  <div class="logo">
    ...
    Configuración de Planes - Empleos
  </div>
  <div class="nav-buttons">
    ...
    <a class="nav-btn" href="../jobs/dashboard.php">
