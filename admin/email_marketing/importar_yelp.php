<?php
/**
 * Importar Lugares desde Yelp Fusion API
...
try {
    ...
    if ($table_exists) {
        $total_lugares = $pdo->query("SELECT COUNT(*) FROM lugares_yelp")->fetchColumn();
        ...
        $last_update_row = $pdo->query("SELECT MAX(updated_at) as last_update FROM lugares_yelp")->fetch();
    ...
    if (file_exists($yelp_config_file)) {
        ...
        $api_key_configured = !empty($yelp_config['api_key']);
    }
} catch (Exception $e) {
    $table_exists = false;
}
...
<div class="row">
    <div class="col-12">
        <h2><i class="fab fa-yelp" style="color: #d32323;"></i> Importar desde Yelp</h2>
        <p class="text-muted">Obtén datos de negocios desde Yelp Fusion API (500 llamadas/día gratis) - <strong>Incluye teléfonos!</strong></p>

        <!-- Navegación de pestañas -->
        <ul class="nav nav-tabs mb-4">
            <li class="nav-item">
                <a class="nav-link active" href="?page=importar-yelp">
                    <i class="fas fa-cloud-download-alt"></i> Importar
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="?page=lugares-yelp">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="alert alert-warning">
                    ...
                    Debes crear la tabla <code>lugares_yelp</code> antes de importar datos.
                </div>

                <button id="btnCrearTabla" class="btn btn-primary btn-lg">
                    <i class="fas fa-database"></i> Crear Tabla lugares_yelp
                </button>
...
<div class="row mb-4">
    <div class="col-12">
        <div class="card">
            <div class="card-header" style="background: linear-gradient(135deg, #d32323 0%, #af1d1d 100%);">
                <h5 class="mb-0 text-white"><i class="fas fa-key"></i> Configurar API Key de Yelp</h5>
            </div>
            <div class="card-body">
                <?php if ($api_key_configured): ?>
                <div class="alert alert-success">
                    <i class="fas fa-check-circle"></i> API Key configurada correctamente
                </div>
                <?php else: ?>
                <div class="alert alert-warning">
                    <i class="fas fa-exclamation-triangle"></i> Necesitas configurar tu API Key de Yelp.
                    <br><a href="https://www.yelp.com/developers/v3/manage_app" target="_blank">Obtener API Key gratis aquí</a>
                </div>
                <?php endif; ?>

                <div class="input-group mb-3">
                    <input type="password" id="yelpApiKey" class="form-control"
                           placeholder="Ingresa tu Yelp API Key" <?= $api_key_configured ? 'value="********"' : '' ?>>
                    <button id="btnGuardarApiKey" class="btn btn-outline-danger">
                        <i class="fas fa-save"></i> Guardar
                    </button>
                </div>
                <small class="text-muted">
                    La API Key de Yelp permite 500 llamadas diarias gratis.
                    <a href="https://www.yelp.com/developers/documentation/v3/get_started" target="_blank">Ver documentación</a>
                </small>
            </div>
        </div>
    </div>
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0"><i class="fas fa-download"></i> Importar Lugares desde Yelp</h5>
            </div>
            <div class="card-body">
                <div class="alert alert-success">
                    <h5><i class="fas fa-star"></i> Ventajas de Yelp sobre Foursquare</h5>
                    <div class="row">
                        <div class="col-md-6">
                            <ul class="mb-0">
                                <li><strong>Teléfonos verificados</strong> incluidos!</li>
                                <li><strong>Ratings</strong> y número de reviews</li>
                                <li><strong>Fotos</strong> de los negocios</li>
                                <li><strong>Precios</strong> ($$, $$$)</li>
                            </ul>
                        </div>
                        <div class="col-md-6">
                            <ul class="mb-0">
                                <li><strong>URLs</strong> de páginas de Yelp</li>
                                <li><strong>Estado</strong> (abierto/cerrado)</li>
                                <li><strong>Categorías</strong> detalladas</li>
                                <li><strong>Direcciones</strong> completas</li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        ...
                        <div id="listaCategorias">
                            <?php foreach ($categorias_yelp as $id => $nombre): ?>
                            <div class="form-check">
                ...
                <div class="text-center my-4">
                    <button id="btnImportar" class="btn btn-lg"
                            style="padding: 20px 50px; font-size: 20px; background: linear-gradient(135deg, #d32323 0%, #af1d1d 100%); color: white; border: none;"
                            <?= (!$table_exists || !$api_key_configured) ? 'disabled' : '' ?>>
                        <i class="fab fa-yelp fa-2x"></i><br>
                        <span style="font-size: 24px; font-weight: bold;">IMPORTAR DESDE YELP</span><br>
                ...
                <div class="alert alert-warning text-center">
                    <i class="fas fa-exclamation-triangle"></i>
                    Primero configura tu API Key de Yelp.
                </div>
                <?php endif; ?>
                    ...
                    <div class="alert alert-info">
                        ...
                        <div class="progress" style="height: 30px;">
                            <div id="progressBar" class="progress-bar progress-bar-striped progress-bar-animated"
                                 role="progressbar" style="width: 0%; font-size: 16px; font-weight: bold; background: #d32323;">0%</div>
                        </div>
                        <p id="progressMessage" class="mt-3 mb-0">Iniciando conexión con Yelp API...</p>
                    </div>
                </div>
...
document.getElementById('btnGuardarApiKey').addEventListener('click', async function() {
    const apiKey = document.getElementById('yelpApiKey').value;
    ...
    try {
        const response = await fetch('/admin/email_marketing/importar_yelp_api.php', {
            method: 'POST',
...
document.getElementById('btnImportar').addEventListener('click', async function() {
    ...
    if (!confirm(`¿Deseas importar lugares de Yelp?\n\nSe realizarán ${totalBusquedas} búsquedas.\nRecuerda: Yelp tiene límite de 500 llamadas/día.\nEsto puede tomar varios minutos.`)) {
        return;
    }
    ...
    updateProgress(5, 'Iniciando importación desde Yelp...');

    let progressInterval = setInterval(async () => {
        try {
            const progressResponse = await fetch('/admin/email_marketing/yelp_progreso_api.php');
        ...
        if (result.success) {
            ...
            setTimeout(() => {
                ...
                resultArea.innerHTML = `
                    <div class="alert alert-success">
                        <h4><i class="fas fa-check-circle"></i> ¡Importación desde Yelp Exitosa!</h4>
                        <hr>
                        <div class="row">
                    ...
                    <div class="text-center mt-3">
                        <a href="?page=lugares-yelp" class="btn btn-primary">
