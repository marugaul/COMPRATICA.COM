...
if (isset($_POST['toggle_affiliate'])) {
    ...
    if ($aff_id > 0) {
        ...
        if ($a && $new_state === 1) {
            ...
            $subject = "✅ Tu cuenta de afiliado ha sido activada";
            $body = "
            <p>Hola <strong>" . htmlspecialchars($a['name']) . "</strong>,</p>
            <p>¡Tu cuenta de afiliado en <strong>" . APP_NAME . "</strong> ha sido activada!</p>
            <p>Ya puedes iniciar sesión y publicar tus espacios de venta:</p>
            <p><a href='" . htmlspecialchars($loginUrl, ENT_QUOTES, 'UTF-8') . "' style='background:#2563eb;color:#fff;padding:10px 18px;border-radius:8px;text-decoration:none;'>Iniciar sesión</a></p>
            <p>Si tienes dudas, contáctanos en <a href='mailto:" . ADMIN_EMAIL . "'>" . ADMIN_EMAIL . "</a>.</p>
            <br><p>— El equipo de " . APP_NAME . "</p>";

            try {
                send_mail($a['email'], $subject, $body);
                send_mail(
                    ADMIN_EMAIL,
                    "📢 Nuevo afiliado activado: {$a['name']}",
                    "<p>Se ha activado el afiliado <strong>{$a['name']}</strong> (<a href='mailto:{$a['email']}'>{$a['email']}</a>) con teléfono <strong>{$a['phone']}</strong>.</p>"
                );
            } catch (Throwable $e) {
                error_log("No se pudo enviar correo de activación: " . $e->getMessage());
            }
        }
    }
}
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      <td><?= (int)$p['id'] ?></td>
      <td><?php if($p['image']): ?><img src="../uploads/<?= htmlspecialchars($p['image']) ?>" class="thumb"><?php endif; ?><?php if(!empty($p['image2'])): ?><img src="../uploads/<?= htmlspecialchars($p['image2']) ?>" class="thumb" style="margin-left:4px"><?php endif; ?></td>
      <td><?= htmlspecialchars($p['name']) ?></td>
      <td><?= $sym . number_format((float)$p['price'], $sym==='$'?2:0, ',', '.') ?></td>
      <td><?= htmlspecialchars($cur) ?></td>
      <td><?= (int)$p['stock'] ?></td>
      <td><?= $p['active'] ? 'Sí' : 'No' ?></td>
      <td><?= htmlspecialchars($p['sale_title'] ?: '—') ?></td>
      <td><?= htmlspecialchars($p['aff_email'] ?: '—') ?></td>
      <td><?= htmlspecialchars($p['fee_status'] ?: '—') ?></td>
      <td><?= $p['sale_active'] ? 'Sí' : 'No' ?></td>
      <td>
        <button class="btn" onclick='fillForm(<?= json_encode($p) ?>)'>Editar</button>
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><?= (int)$o['qty'] ?></td>
      <td><?= htmlspecialchars($o['buyer_email'] . " / " . $o['buyer_phone']) ?></td>
      <td><?= htmlspecialchars($o['residency']) ?></td>
      <td><?= htmlspecialchars($o['status']) ?></td>
      <td>
        <?php if($o['proof_image']): ?>
          <a href="../uploads/payments/<?= htmlspecialchars($o['proof_image']) ?>" target="_blank">
            <img class="thumb" src="../uploads/payments/<?= htmlspecialchars($o['proof_image']) ?>">
          </a>
        <?php else: ?>
