...
if (($_GET['action'] ?? '') === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    logCart('ADD_PARAMS_RECEIVED', ['pid' => $pid, 'sale_id' => $sale_id, 'qty' => $qty]);
    
    if ($pid <= 0 || $qty <= 0) {
        logCart('ADD_ERROR', ['msg' => 'Invalid params', 'pid' => $pid, 'qty' => $qty]);
    ...
    try {
        ...
        // Obtener precio y stock del producto desde la BD
        $st = $pdo->prepare("SELECT price, sale_id, stock FROM products WHERE id=? LIMIT 1");
        $st->execute([$pid]);
        $productData = $st->fetch(PDO::FETCH_ASSOC);
        
        if (!$productData) {
            logCart('ADD_ERROR', ['msg' => 'Producto no encontrado', 'pid' => $pid]);
            echo json_encode(['ok' => false, 'error' => 'Producto no encontrado']);
            exit;
        }
        
        $unit = (float)$productData['price'];
        $stock_disponible = (int)$productData['stock'];
        
        if ($sale_id <= 0) {
            $sale_id = (int)$productData['sale_id'];
        ...
        if ($existing) {
            ...
            if ($new_qty > $stock_disponible) {
                logCart('ADD_ERROR_STOCK', ['pid' => $pid, 'requested' => $new_qty, 'available' => $stock_disponible]);
                echo json_encode(['ok' => false, 'error' => "Stock insuficiente. Disponible: {$stock_disponible}"]);
                exit;
            }
            ...
            if ($qty > $stock_disponible) {
                logCart('ADD_ERROR_STOCK', ['pid' => $pid, 'requested' => $qty, 'available' => $stock_disponible]);
                echo json_encode(['ok' => false, 'error' => "Stock insuficiente. Disponible: {$stock_disponible}"]);
                exit;
            }
...
if (($_GET['action'] ?? '') === 'update' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($qty <= 0) {
            ...
            // Validar stock disponible
            $st = $pdo->prepare("SELECT stock FROM products WHERE id=? LIMIT 1");
            $st->execute([$pid]);
            $stock_disponible = (int)$st->fetchColumn();
            
            if ($qty > $stock_disponible) {
                logCart('UPDATE_ERROR_STOCK', ['pid' => $pid, 'requested' => $qty, 'available' => $stock_disponible]);
                echo json_encode(['ok' => false, 'error' => "Stock insuficiente. Disponible: {$stock_disponible}"]);
                exit;
            }
