...
/**
 * ajax_uber_quote.php
 * Endpoint AJAX para obtener cotización de envío por Uber
 * ✨ INCLUYE MODO SANDBOX para pruebas sin credenciales de Uber
 */
...
try {
    ...
    if (!$pickup) {
        echo json_encode([
            'success' => false, 
            'message' => 'No se encontró ubicación de recogida configurada para este espacio'
        ]);
        exit;
    }
    ...
    if (empty($pickup['lat']) || empty($pickup['lng'])) {
        echo json_encode([
            'success' => false,
            'message' => 'La ubicación de recogida no tiene coordenadas configuradas. El vendedor debe completar esta información.'
        ]);
        exit;
    }
    ...
    $response = [
        ...
        // 🎮 Indicar si está en modo sandbox
        'is_sandbox' => $is_sandbox,
        'sandbox_reason' => $is_sandbox ? $sandbox_reason : null
    ];
    
    // Guardar cotización temporalmente en sesión para usarla en el checkout
