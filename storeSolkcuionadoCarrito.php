...
if ($sale_id < 1) {
  try {
    ...
    if (count($rows) === 1) {
      $sale_id = (int)$rows[0];
      log_error_cart("Auto-resuelto sale_id único activo: $sale_id");
    }
  } catch (Throwable $e) {
...
<head>
  ...
  <style>
    ...
    .badge { background:#222;color:#fff;border-radius:999px;padding:2px 8px;font-size:.8rem;display:none; }
  </style>
</head>
<body>
<header class="header">
  <div class="logo">&#128722; <?= APP_NAME ?> <span id="cartBadge" class="badge"></span></div>
  <nav>
    ...
    <?php foreach ($products as $p): ?>
      <div class="card">
        ...
        <?php if ((int)$p['stock'] > 0): ?>
          <!-- IMPORTANTE: sin onsubmit inline; method=post para no filtrar datos por GET si fallara el JS -->
...
<!-- Add-to-Cart: listeners delegados (sin inline) + badge -->
<script>
(function(){
  var API = '<?= $API_URL ?>';
  ...
  async function addToCart(form){
    ...
    var csrfToken = getCsrf() || (form.querySelector('input[name="csrf_token"]')?.value || '');
    ...
    try{
      ...
      if(!res.ok){
        ...
        alert('❌ No se pudo agregar (HTTP '+res.status+').');
        return;
      }
  ...
  // Interceptar submit de TODOS los formularios .atc
  ...
  (async function(){
    try{
      var r = await fetch(API+'?action=get', {credentials:'same-origin'});
