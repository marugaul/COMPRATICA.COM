...
$stmt = $pdo->prepare("SELECT id FROM job_listings WHERE id = ? AND employer_id = ? AND listing_type IN ('job', 'service')");
...
try {
  $stmt = $pdo->prepare("DELETE FROM job_listings WHERE id = ? AND employer_id = ? AND listing_type IN ('job', 'service')");
  $stmt->execute([$listing_id, $employer_id]);
