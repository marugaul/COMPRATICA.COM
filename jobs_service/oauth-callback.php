...
if (empty($state) || empty($expectedState) || !hash_equals($expectedState, $state)) {
    ...
    header('Location: /jobs_service/register.php?error=' . urlencode('Estado de seguridad inválido.'));
    exit;
}

$code = $_GET['code'] ?? '';
if (empty($code)) {
    header('Location: /jobs_service/register.php?error=' . urlencode('Código de autorización no recibido.'));
    exit;
}
...
if (empty($GOOGLE_CLIENT_ID) || empty($GOOGLE_CLIENT_SECRET)) {
    header('Location: /jobs_service/register.php?error=' . urlencode('Credenciales de Google no configuradas.'));
    exit;
}

try {
    $redirectUri = rtrim(SITE_URL, '/') . '/jobs_service/oauth-callback.php';
    ...
    header('Location: /jobs_service/dashboard.php?login=success');
    exit;
    ...
    header('Location: /jobs_service/register.php?error=' . urlencode($userError));
    exit;
}
